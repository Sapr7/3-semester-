...
public:
    ...
    void printLogs(const std::string& filename) const {
...
public:
    ...
    explicit Particle(double energy, double mass_, double charge_) : mass(mass_), charge(charge_) {
        coordinate = randv3();
...
public:
    void spawnPrticle(int type) {
        ...
        switch (type) {
            case 0:
                p = new Electron(randd());
                break;
            case 1:
                p = new Proton(randd());
                break;
            case 2:
                p = new Positron(randd());
                break;
        }
        if (p) {
            v.push_back(p);
